...
Probes were designed using an interval size of 100bp and a window size of 25bp.  Location of probes can be ascertained by visualizing the generic feature format file: `/tracks/OID40453_probe_locations.gff`.  Two different comparative hybridizations were performed. The EE<sub>2</sub> exposed and control methylated enriched samples labels were swapped between replicates.  A third hybridization was performed using the input DNA from the EE<sub>2</sub> and control samples to control for variation in signal that resulted from copy number variation or other artifact. In other words, DNA was used prior to any enrichment.  Samples were labeled using the NimbleGen Dual-Color DNA Labeling Kit and the arrays were processed according to the manufacturer’s recommendations (Roche NimbleGen, Madison, Wisconsin) and imaged at 5um using a GenePix 4000B microarray scanner (Molecular Devices, Sunnyvale, CA).
